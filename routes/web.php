...
Route::middleware('auth')->group(function () {
    ...
    Route::middleware('isAccess:handler-op,agent-op')->group(function () {
        Route::prefix('secretaire')->group(function () {
            Route::get('/dashboard', [UserController::class, 'index'])->name('customersDashboard');
            Route::resource('/utilisateurs', CustomerController::class);

            Route::resource('/users', AdminController::class, [
                'names' => array(
                    'index' => 'secretaire.users.index',
                    'create' => 'secretaire.users.create',
                    'show' => 'secretaire.users.show',
                    'edit' => 'secretaire.users.edit',
                    'destroy' => 'secretaire.users.destroy',
                ),
                'except' => ['store', 'update'],
            ]);
            Route::resource('/utilisateurs', AccountController::class, [
                'names' => array( 
                    'edit' => 'secretaire.utilisateurs.edit',
                    'show' => 'secretaire.utilisateurs.show',
                    'update' => 'secretaire.utilisateurs.update',
                ),
                'except' => ['index', 'create', 'destroy', 'store'],
            ]);
        });
